...
export default function DtmPremium() {
  ...
  const aiAnalysis = useMemo(() => {
    ...
    if (total >= 170)
      feedback =
        "Siz mutloq rekordchisiz! Bilimingiz darajasi maksimal cho'qqida.";
    else if (total >= 140)
      feedback = "Ajoyib natija! Grant uchun imkoniyatingiz juda yuqori.";
    else if (total >= 100)
      feedback = "Yaxshi ko'rsatkich. Kamchiliklar ustida ko'proq ishlang.";
    else
      feedback =
        "Sizda ulkan salohiyat bor. Har bir test sizni orzungizga yaqinlashtiradi.";
    return feedback;
  }, [currentTest]);
  ...
  const downloadPDF = async () => {
    ...
    try {
      ...
      pdf.save(`DTM_${student.name}.pdf`);
    } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] dark:bg-[#050505] pt-24 pb-10 px-4 lg:px-8 font-sans transition-all duration-500 overflow-x-hidden">
      <div className="max-w-[1600px] mx-auto w-full flex flex-col gap-6">
        {/* HEADER / SEARCH (PDF'da ko'rinmaydi) */}
        <div
          data-html2canvas-ignore="true"
          className="flex flex-col lg:flex-row justify-between items-center gap-6 shrink-0"
        >
        ...
        >
          ...
          <div className="order-1 lg:order-3 lg:col-span-4 flex flex-col gap-6">
            <div className="bg-[#0f172a] p-8 rounded-[2.5rem] text-white shadow-2xl relative overflow-hidden border border-[#39B54A]/20 transition-transform hover:scale-[1.01]">
            ...
            {/* AI ANALYSIS CARD */}
            <div className="bg-white dark:bg-zinc-900 p-8 rounded-[2.5rem] shadow-xl border border-white dark:border-zinc-800 flex flex-col">
              ...
              <div className="space-y-6">
                <div className="flex justify-between items-center bg-slate-50 dark:bg-zinc-800 p-4 rounded-2xl border dark:border-zinc-700">
                ...
                <p className="text-sm text-slate-600 dark:text-zinc-300 font-medium italic border-l-4 border-[#39B54A] pl-4 leading-relaxed">
                  "{aiAnalysis}"
                </p>
                <button
                  onClick={downloadPDF}
                  disabled={isDownloading}
                  className="w-full py-4 bg-black dark:bg-[#39B54A] text-white dark:text-black rounded-3xl font-black flex items-center justify-center gap-2 hover:opacity-80 active:scale-95 transition-all shadow-xl shadow-[#39B54A]/20"
                >
                  {isDownloading ? (
        ...
        {/* HISTORY (PDF'da ko'rinmaydi) */}
        <div
          data-html2canvas-ignore="true"
          className="h-16 flex items-center gap-3 px-6 bg-white dark:bg-zinc-900 rounded-2xl border dark:border-zinc-800 overflow-x-auto no-scrollbar shadow-sm"
        >
