    ...
    <div className="bg-white dark:bg-[#050505] transition-colors min-h-screen font-sans overflow-x-hidden pt-24 pb-20">
      ...
      <section className="max-w-7xl mx-auto px-6 py-24 border-t dark:border-zinc-900 text-center">
        ...
        <div className="flex gap-4 mt-12 justify-center">
          ...
          <button
            onClick={() => handleManualMove("teacher", "next")}
            className="p-3 border-2 rounded-full dark:border-zinc-800 dark:text-white hover:bg-[#39B54A] transition-colors"
          >
            <ChevronRight size={20} />
          </button>
      ...
      <AnimatePresence>
        {selected && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-[2000] flex items-center justify-center bg-black/90 dark:bg-black/95 light:bg-zinc-100/80 backdrop-blur-2xl p-4 lg:p-10"
          >
            <motion.div
              ...
              className="relative w-full max-w-5xl h-[90vh] rounded-[3.5rem] overflow-hidden flex flex-col dark:bg-white/5 dark:backdrop-blur-2xl dark:border dark:border-white/20 dark:shadow-[0_80px_200px_rgba(0,0,0,0.9)] bg-white border border-zinc-200 shadow-[0_40px_120px_rgba(0,0,0,0.15)]"
            >
              {/* CLOSE */}
              <button
                onClick={() => setSelected(null)}
                className="absolute top-6 right-6 z-30 p-4 rounded-full transition-all duration-300 dark:bg-white/20 dark:text-white dark:hover:rotate-90 dark:hover:scale-110 bg-zinc-100 text-zinc-800 hover:bg-zinc-200"
              >
                <X />
              </button>

              {/* IMAGE */}
              <div className="relative h-[45%] shrink-0 flex items-center justify-center dark:bg-[radial-gradient(circle_at_center,rgba(255,255,255,0.18),transparent_70%)] bg-gradient-to-br from-zinc-100 to-zinc-200 border-b border-zinc-200 dark:border-white/10">
                <img
                  src={selected.img}
                  alt={selected.name}
                  className="h-full object-contain relative z-10 dark:drop-shadow-[0_50px_120px_rgba(0,0,0,0.85)] drop-shadow-[0_20px_60px_rgba(0,0,0,0.25)]"
                />
              </div>

              {/* CONTENT */}
              <div className="flex-1 px-8 lg:px-14 py-6 flex flex-col gap-6 overflow-hidden">
                <h2 className="text-center text-4xl md:text-6xl font-black uppercase italic tracking-tighter dark:text-white text-zinc-900">
                  {selected.name}
                </h2>
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                  ...
                  <div className="modal-stat">
                    <Briefcase size={18} />
                    <span>{selected.experience || "Tajriba yo'q"}</span>
                  </div>
                  <div className="modal-stat">
                    <Award size={18} />
                    <span>{selected.achievements || "Yutuqlar yo'q"}</span>
                  </div>
                </div>

                <div className="flex-1 min-h-0 grid grid-cols-1 lg:grid-cols-12 gap-8 items-center pt-6 border-t border-zinc-200 dark:border-white/10">
                  <div className="lg:col-span-4">
                    <h5 className="text-[10px] uppercase tracking-widest mb-2 flex items-center gap-2 text-zinc-500 dark:text-white/50">
                      <GraduationCap size={14} /> Tahsil
                    </h5>
                    <p className="text-sm font-bold line-clamp-3 text-zinc-800 dark:text-white/90">
                      {selected.education}
                    </p>
                  </div>
                  <div className="lg:col-span-8 relative pl-0 lg:pl-8 lg:border-l border-zinc-200 dark:border-white/10">
                    <Quote
                      size={46}
                      className="absolute -top-4 left-4 text-zinc-200 dark:text-white/10 hidden lg:block"
                    />
                    <p className="pl-10 italic leading-relaxed line-clamp-4 text-zinc-600 dark:text-white/70">
                      “{selected.bio}”
                    </p>
                  </div>
                </div>
